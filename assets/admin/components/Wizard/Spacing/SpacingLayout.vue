...
import SpacingItem from './SpacingItem.vue';
...
const newSpacing = ref({
    key: null,
    value: null,
});

function addNewSpacing() {
    // if property is not defined, define it
    if (Object.prototype.hasOwnProperty.call(selectedWizard.value.preset, 'spacing') === false) {
    ...
    selectedWizard.value.preset.spacing.push({
        id: nanoid(10),
        key: newSpacing.value.key,
        value: newSpacing.value.value,
    });
...
function generateFluidSpacing(config) {
    if (Object.prototype.hasOwnProperty.call(selectedWizard.value.preset, 'spacing') === false) {
        selectedWizard.value.preset = {
            ...selectedWizard.value.preset,
            spacing: [],
            ...
            <div class="grid grid-template-cols:1fr|0.5fr">
                <div>
                    <div class="grid uppercase f:10 fg:gray-90 gap:10 grid-template-columns:minmax(6.25rem,1fr)|minmax(12.5rem,2fr) ls:2px mb:-16 pt:28 px:10">
                        <span class="opacity:.5">Spacing name</span>
                        <span class="opacity:.5 pl:22">Spacing Size</span>
                    </div>

                    <div class="grid px:8 py:28">
                        <div class="new-spacing grid rel gap:10 grid-template-columns:15px|minmax(6.25rem,1fr)|minmax(12.5rem,2fr) p:12">
                            ...
                            <div class="spacing-key">
                                <div class="rel">
                                    ...
                                    <input type="text" v-model="newSpacing.key" name="spacing-key" id="spacing-key" class="block border:1|solid|gray-20 fg:gray-90 fg:gray-30::placeholder py:6 r:6 w:full" placeholder="lg">
                                </div>
                            </div>
                            <div class="grid gap:20 grid-template-columns:minmax(7rem,2fr)|minmax(3rem,1fr)">
                                <div class="rel">
                                    ...
                                    <input type="text" v-model="newSpacing.value" name="spacing-value" id="spacing-value" class="block border:1|solid|gray-20 fg:gray-90 fg:gray-30::placeholder py:6 r:6 w:full" placeholder="16px">
                                </div>
                                <button @click="addNewSpacing" :disabled="!newSpacing.key || !newSpacing.value" class="button align-items:center bg:sky-80/.15 bg:sky-80/.3:hover border:0 fg:sky-80 float:right ml:0 px:10 r:8 text:center">
                        ...
                        <Draggable v-model="selectedWizard.preset.spacing" tag="transition-group" item-key="id" ghost-class="dragged-placeholder" handle=".spacing-item-drag-handle" animation="200">
                            <template #item="{ element }">
                                <div class="spacing-item">
                                    <SpacingItem :spacing="element" />
                                </div>
                            </template>
                        </Draggable>
                    </div>
                ...
                <FluidCalculator @calculate="generateFluidSpacing" />
            </div>
