...
async function constructVariableList() {
  const vfsContainer = bdeIframe.contentWindow.document.querySelector(
...
watch(focusedInput, (value) => {
  if (value) {
    ...
    const isColorInput = ["color"].some((keyword) =>
      control.getAttribute("data-test-id").includes(keyword),
    );
    const isFontSize = ["fontSize"].some((keyword) =>
      control.getAttribute("data-test-id").includes(keyword),
    );
    const isSpacing = ["spacing", "size-width"].some((keyword) =>
      control.getAttribute("data-test-id").includes(keyword),
    );
    ...
    async function swithUnitCustom() {
      while (
        value.parentElement.parentElement.parentElement.querySelector(
          "div.dropdown>button.breakdance-unit-input-unit",
        ) === null
      ) {
      ...
      value.parentElement.parentElement.parentElement
        .querySelector("div.dropdown>button.breakdance-unit-input-unit")
        .click();

      while (
        document.querySelector(
          ".v-menu__content.menuable__content__active .dropdown-content .v-list .v-list-item:last-child .v-list-item__title",
        ) === null
      ) {
        await new Promise((resolve) => setTimeout(resolve, 10));
      }
