...
<template>
    <DraggableTreeItem :item="item" :level="level" :has-children="!!(item.children && item.children.length > 0)" :is-last="false" :is-descendant-of="isDescendantOf" :on-delete="onDelete" :class="{
        ...
        <div class="flex items-center gap-2 w-full">
            ...
            <slot name="leading" :item="item"></slot>
            <div>
            ...
            <slot name="before-value" :item="item"></slot>
            <div class="w-full">
                <UInput v-model="item.var.value" @update:model-value="(value) => { item.var.value = value }" @keydown.left="handleLeftRightArrowKey" @keydown.right="handleLeftRightArrowKey" placeholder="" :ui="{ base: 'peer', root: 'block' }">
                    <label class="pointer-events-none absolute left-0 -top-2.5 text-highlighted text-xs font-medium px-1.5 transition-all peer-focus:-top-2.5 peer-focus:text-highlighted peer-focus:text-xs peer-focus:font-medium peer-placeholder-shown:text-sm peer-placeholder-shown:text-dimmed peer-placeholder-shown:top-1.5 peer-placeholder-shown:font-normal" :class="{
                        'peer-placeholder-shown:ps-9 peer-focus:ps-1.5': $slots['value-leading'],
                    }">
                        <span class="inline-flex bg-default px-1">{{ i18n.__('Value') }}</span>
                    </label>

                    <template v-if="$slots['value-leading']" #leading>
                        <slot name="value-leading" :item="item"></slot>
                    </template>
                </UInput>
            </div>
            <div>
