...
<template>
  <TreeItem ref="elRef" v-slot="{ isExpanded }" :value="item.value" :level="item.level"
    class="relative w-full border-none" :class="{ 'opacity-50': isDragging }">
    <Icon v-if="item.hasChildren" icon="radix-icons:chevron-down" class="h-4 w-4 transition"
      :class="{ '-rotate-90': isExpanded }" />
    <div class="pl-2"> {{ item.value.title }} </div>
    <div v-if="instruction" class="absolute h-full w-full top-0 border-blue-500 " :style="{
