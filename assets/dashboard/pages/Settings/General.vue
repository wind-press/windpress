...
<template>
  ...
  >
    <UForm
      :state="license"
      @submit="doLicenseChange"
    >
      <UFormField
        label="License key"
        required
        :error="licenseKeyError"
        help="To access updates when they are available, please provide your license key."
      >
        <div class="flex flex-row gap-4 my-2">
          ...
          <UButton
            type="submit"
            color="neutral"
            :leading-icon="busyStore.isBusy && busyStore.tasks.some((t) => t.task === 'settings.license.activate' || t.task === 'settings.license.deactivate') ? 'lucide:loader-circle' : undefined"
        ...
        <template
          v-if="licenseStore.license.key"
          #hint
        >
          <div class="flex items-center gap-2">
            <span class="text-(--ui-text-muted)">Status:</span>
            <UBadge
              :color="licenseStore.isActivated ? 'success' : 'error'"
              variant="soft"
            >
  ...
  >
    <UPageCard
      title="General"
      description="General settings for WindPress."
      variant="naked"
    ...
    <UPageCard variant="subtle">
      <UFormField
        ...
        description="You must update the `main.css` file accordingly."
